...
(deftest sample-4-test
  (testing "Пример из условия: максимальное произведение 4 подряд цифр = 5832"
    ;; используем str и проверяем, что для окна 4 получаем 5832
    (is (= 5832 (recursive str 4)))
    (is (= 5832 (tailrec str 4)))
    (is (= 5832 (mapindexed str 4)))
    (is (= 5832 (lazy str 4)))))

(deftest full-13-test
  (testing "Полный 1000-значный пример: ожидаем 23514624000"
    (let [expected 23514624000]
      (is (= expected (recursive str 13)))
      (is (= expected (tailrec str 13)))
      (is (= expected (mapindexed str 13)))
      (is (= expected (lazy str 13))))))
